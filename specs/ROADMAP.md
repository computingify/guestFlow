...
## Bloc 2 — Pricing engine 🟡 (live goals met)
...
- ⬜ Price denormalization (carried from Bloc 0): **own later spec** (decided), not in this bloc.
...
## Bloc 3 — Réservations & Calendar ⬜ (largest)
...
## Bloc 6 — Google Calendar, iCal & Holidays ⬜
...
## Bloc S — Security hardening ⬜ (cross-cutting, CRITICAL)
...
1. **Bloc S (auth + encryption first)** if the app is network-exposed — otherwise weave security into each bloc.
2. **Bloc 2 (Pricing)** — unblocks Bloc 3 & 5 by making the server the price authority.
3. **Bloc 3 (Réservations & Calendar)** — biggest payoff, depends on Bloc 2.
4. **Bloc 4 (Devis)**, **Bloc 5 (Finance)**, **Bloc 1 (Clients/Resources)**, **Bloc 6 (Integrations)** — order flexible.

> Order is a recommendation, not a contract. We pick the next bloc together at the start of each cycle.
