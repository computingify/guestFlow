...
router.post('/', upload.single('photo'), (req, res) => {
  const { name, maxAdults, maxChildren, maxBabies, depositPercent, depositDaysBefore, balanceDaysBefore, defaultCheckIn, defaultCheckOut, cleaningHours, defaultCautionAmount } = req.body;
  const photo = req.file ? `/uploads/${req.file.filename}` : '';
  const result = db.prepare(`
    INSERT INTO properties (name, photo, maxAdults, maxChildren, maxBabies, depositPercent, depositDaysBefore, balanceDaysBefore, defaultCheckIn, defaultCheckOut, cleaningHours, defaultCautionAmount)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(name, photo, maxAdults || 2, maxChildren || 0, maxBabies || 0, depositPercent || 30, depositDaysBefore || 30, balanceDaysBefore || 7, defaultCheckIn || '15:00', defaultCheckOut || '10:00', cleaningHours || 3, defaultCautionAmount ?? 500);
...
router.put('/:id', upload.single('photo'), (req, res) => {
  ...
  db.prepare(`
    UPDATE properties SET name=?, photo=?, maxAdults=?, maxChildren=?, maxBabies=?, depositPercent=?, depositDaysBefore=?, balanceDaysBefore=?, defaultCheckIn=?, defaultCheckOut=?, cleaningHours=?, defaultCautionAmount=?, updatedAt=datetime('now')
    WHERE id=?
  `).run(name, photo, maxAdults || 2, maxChildren || 0, maxBabies || 0, depositPercent || 30, depositDaysBefore || 30, balanceDaysBefore || 7, defaultCheckIn || '15:00', defaultCheckOut || '10:00', cleaningHours || 3, defaultCautionAmount ?? 500, req.params.id);
  res.json({ ok: true });
});
